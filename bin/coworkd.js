...
async function main() {
  ...
  if (!fs.existsSync(mainPath)) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        'Build failed. Run "npm run build:electron && npm run build:connectors" manually.',
      );
  ...
  if (!ok) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "Failed to rebuild better-sqlite3 for Electron. Ensure build tools are installed.",
      );
      // eslint-disable-next-line no-process-exit
      process.exit(1);
    }
