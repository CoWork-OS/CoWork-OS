...
function projectRoot(): string {
  ...
  if (!raw.trim()) {
    throw new Error(
      "COWORK_ARCH_PROJECT_ROOT or COWORK_WORKSPACE_ROOT is required for ComfyUI file path tools.",
    );
  }
...
async function fetchComfyBinary(url: string, label: string): Promise<Buffer> {
  ...
  if (!response.ok) {
    const data = await parseResponse(response);
    throw new Error(
      data?.error || data?.message || `Failed to fetch ComfyUI output ${label}: ${response.status}`,
    );
  }
...
const tools: MCPTool[] = [
  {
    name: "comfyui.health",
    description: "Check ComfyUI API availability",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "comfyui.list_workflows",
    description: "List local workflow JSON files from COMFYUI_WORKFLOW_DIR",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "comfyui.submit_workflow",
    description: "Submit a ComfyUI workflow JSON graph",
    inputSchema: {
      type: "object",
      properties: {
        workflow: objectSchema("ComfyUI prompt/workflow graph"),
  ...
  },
  {
    ...
    description: "Submit a Flux-style photoreal workflow graph with project metadata",
    inputSchema: {
      type: "object",
      properties: {
        workflow: objectSchema(
          "ComfyUI workflow graph with {{prompt}}, {{negativePrompt}}, {{sourceImagePath}}, and {{projectId}} placeholders",
        ),
  ...
  {
    name: "comfyui.get_job_status",
    description: "Get queue/status for a submitted prompt",
    inputSchema: {
      type: "object",
  ...
  {
    name: "comfyui.get_history",
    description: "Get ComfyUI history, optionally for one prompt id",
    inputSchema: {
      type: "object",
      properties: { promptId: { type: "string" } },
      required: [],
  ...
  },
  {
    ...
    description:
      "Collect output image URLs for a completed prompt id and optionally copy them under the project root",
    inputSchema: {
      type: "object",
      properties: {
        ...
        outputDir: {
          type: "string",
          description: "Optional project-root-relative output directory for copied images",
        },
      },
      required: ["promptId"],
      additionalProperties: false,
...
async function listWorkflows(): Promise<any> {
  ...
  if (!workflowDir) {
    return {
      ...
      workflowDir: null,
      workflows: [],
      note: "Set COMFYUI_WORKFLOW_DIR to list saved workflows.",
    };
  }
...
async function submitFluxPhotorealPass(args: Record<string, any>): Promise<any> {
  ...
  if (!workflowText.includes("{{prompt}}")) {
    throw new Error(
      "Flux workflow must include a {{prompt}} placeholder so prompt metadata is applied.",
    );
  }
...
async function getJobStatus(args: Record<string, any>): Promise<any> {
  ...
  const running =
    ...
    queue.queue_running.some((item: any) => JSON.stringify(item).includes(promptId));
  const pending =
    Array.isArray(queue?.queue_pending) &&
    queue.queue_pending.some((item: any) => JSON.stringify(item).includes(promptId));
  return {
    ok: true,
    promptId,
    status: running ? "running" : pending ? "pending" : "not_in_queue",
    queue,
  };
}
