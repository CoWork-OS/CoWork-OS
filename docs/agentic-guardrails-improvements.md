...
| `browser_*`       | `6`         | `10`             |
| `run_command`     | _(not set)_ | `10` (new entry) |
...
Before returning `true` for a detected loop, checks if the wider call window contains a _mix_ of read-like and write-like tool categories:

```typescript
const widerWindow = recentCalls.slice(-(threshold + 3));
const allCategories = new Set(widerWindow.map((c) => c.tool));
const hasReadLike = [...allCategories].some((cat) => /^(read|search|list|glob|find|get)/.test(cat));
...
| `exit code [1-9]`, `non-zero exit`, `exited with` | Clarify the _command_ failed (normal during dev), not the tool; read the error and retry |
...
The method triggered on broad keywords like `"breaking"`, `"news"`, `"search"` even when the task was about _building a tool related to_ those topics, not _fetching_ them. Added build-intent counterweight:
