...
stores consent windows and an outbox locally; the random profile ID/token live in encrypted
settings. Desktop/daemon timers and CLI commands invoke delivery. Shared types, Electron IPC,
onboarding, and the Pulse settings panel expose consent and inspection controls.
...
The desktop updater independently uses an identifier-free version endpoint with a local cache
and GitHub fallback. Operator OTLP export and local Usage Insights remain distinct data flows.
