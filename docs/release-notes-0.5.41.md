...
- **Visible browser workbench**: interactive browser-use tasks now open a live Electron webview in the task sidebar by default, with URL controls, persistent workspace browser partitions, fullscreen mode, visible cursor/action feedback, screenshot capture, screenshot annotation, and follow-up handoff from the same workbench. Browser tools can still fall back to forced headless Playwright, external Chrome profiles, or DevTools attach when explicitly requested. [Learn more](web-page-artifacts.md#relationship-to-browser-workbench)
...
- **Browser tool routing**: `browser_navigate`, click/fill/type/press/scroll/wait/content/evaluate/screenshot, and related browser tools now prefer the visible in-app browser session for user-facing site testing, keep session ids across calls, and report profile-launch conflicts with a visible-workbench retry path.
