...
const QA_WORKSPACE_ID =
  process.env.COWORK_QA_WORKSPACE_ID || "f0e94e20-1c54-4d8b-93fe-700c77ad3258";
const REPO_WORKSPACE_ID =
  process.env.COWORK_REPO_WORKSPACE_ID || "07d63869-c23a-4418-8b25-b0ced9c76b12";
...
async function waitForTerminalStatus(db, taskId, opts) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    for (const approval of approvals) {
      ...
      const resp = await postJson("/hooks/approval/respond", {
...
async function main() {
  ...
  const scenarios = [
    {
      ...
      verify: (abs) => {
        ...
        return content === `hello from battery ${runId}`
          ? { ok: true }
          : { ok: false, error: "content_mismatch", content };
      },
    },
    {
      ...
      verify: (abs) => {
        ...
        return /example domain/i.test(content)
          ? { ok: true }
          : { ok: false, error: "unexpected_title", content };
      },
    },
    {
    ...
    },
    {
      ...
      verify: (abs) => {
        ...
        if (!ok.ok) return ok;
        const content = readTextFile(abs).trim();
        return /^v\d+\./.test(content)
          ? { ok: true }
          : { ok: false, error: "unexpected_node_version", content };
      },
    },
    {
      ...
      verify: (abs) => {
        ...
        if (content.length < 2)
          return { ok: false, error: "missing_line2", content: content.join("\\n") };
  ...
  for (const scenario of scenarios) {
    ...
    const workspacePath =
      workspace && typeof workspace.path === "string" ? workspace.path : process.cwd();
    const outAbs = path.isAbsolute(scenario.outRel)
      ? scenario.outRel
      : path.join(workspacePath, scenario.outRel);
    ...
    console.log(
      `[battery] done: status=${task.status} verify=${verifyResult && verifyResult.ok ? "ok" : "fail"}`,
    );
  }
  ...
  for (const r of results) {
    ...
    if (!r.ok) {
      ...
      if (r.verify && r.verify.error)
        console.log(`  verify: ${JSON.stringify(r.verify).slice(0, 500)}`);
