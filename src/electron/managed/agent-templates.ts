...
function makeStudio(studio: Partial<ManagedAgentStudioConfig>): Partial<ManagedAgentStudioConfig> {
  return studio;
}
...
export const BUILTIN_AGENT_TEMPLATES: AgentTemplate[] = [
  ...
  },
  {
    ...
    studio: makeStudio({
      skills: ["github", "code-review", "debug-error"],
      apps: {
        allowedToolFamilies: ["files", "search", "documents"],
      },
      memoryConfig: { mode: "focused", sources: ["workspace", "sessions"] },
      scheduleConfig: { enabled: false, mode: "manual" },
  ...
  },
  {
    ...
    name: "Chief of Staff",
    description:
      "Prepare executive-style briefs from inbox, calendar, chats, and workspace context.",
    icon: "🧳",
    color: "#14b8a6",
    category: "operations",
    systemPrompt:
  ...
  },
  {
    ...
    category: "finance",
    featured: true,
    systemPrompt: `You coordinate a finance pitch workflow. Build an evidence-backed source ledger, prepare analysis workpapers, draft a presentation outline, generate a reviewable PPTX artifact, and stop for sign-off before any external use. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
  ...
  },
  {
    ...
    description:
      "Prepare concise finance meeting briefs with sources, open questions, and follow-up risks.",
    icon: "🗓️",
    color: "#2563eb",
    category: "finance",
    systemPrompt: `Prepare meeting briefs from read-only company, market, transcript, note, and document context. Return a short agenda, participant context, source-backed discussion points, unresolved questions, and a source-ledger.json artifact. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "solo",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true, webSearchMode: "live" },
  ...
  },
  {
    ...
    color: "#0891b2",
    category: "finance",
    systemPrompt: `Run a read-only market research workflow. Compare sources, identify consensus and dissenting signals, produce a concise memo, and include a source ledger with every material market claim. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true, webSearchMode: "live" },
    skills: [
      "finance-source-ledger",
      "er-sector-analysis",
      "er-catalyst-tracking",
      "er-screen",
      "er-thesis",
    ],
  ...
  },
  {
    ...
    category: "finance",
    featured: true,
    systemPrompt: `Analyze earnings materials and call context. Produce a beat/miss view, guidance bridge, estimate revision notes, sector read-through, source ledger, and review checkpoint before conclusions are reused. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true, webSearchMode: "live" },
    skills: [
      "finance-source-ledger",
      "er-earnings-analysis",
      "er-earnings-preview",
      "er-model-update",
      "er-morning-note",
    ],
  ...
  },
  {
    ...
    color: "#7c3aed",
    category: "finance",
    systemPrompt: `Build finance workbook artifacts with Inputs, calculation tabs, Checks, and a linked Source Ledger. Surface assumption gaps, failed checks, and review checkpoints. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true, webSearchMode: "live" },
    skills: [
      "finance-xlsx-author",
      "finance-model-audit",
      "fa-dcf-modeling",
      "fa-peer-benchmarking",
      "fa-three-statement-model",
      "fa-lbo-modeling",
    ],
  ...
  },
  {
    ...
    color: "#f59e0b",
    category: "finance",
    systemPrompt: `Review valuation artifacts for source support, math integrity, assumption sensitivity, comp relevance, and presentation clarity. Return exceptions, proposed fixes, and a sign-off checklist; do not finalize advice. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true, webSearchMode: "live" },
    skills: [
      "finance-model-audit",
  ...
  },
  {
    ...
    name: "GL Reconciler",
    description:
      "Prepare reconciliation workpapers, trace breaks, and stage exceptions for approval.",
    icon: "🧾",
    color: "#475569",
    category: "finance",
    systemPrompt: `Run a GL reconciliation workflow from provided ledger and support files. Produce reconciliation tables, break tracing notes, source ledger, and a controller review checkpoint. Never post journal entries or mark reconciliation complete without human sign-off. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
  ...
  },
  {
    ...
    color: "#0ea5e9",
    category: "finance",
    systemPrompt: `Prepare month-end close workpapers from provided files and read-only systems. Produce accrual support, roll-forwards, variance commentary, NAV or trial-balance tie-outs where applicable, and approval pauses for controller review. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true },
    skills: [
      "finance-workpaper-manifest",
      "fund-accrual-schedule",
      "fund-roll-forward",
      "fund-variance-commentary",
      "fund-nav-tieout",
    ],
    mcpServers: ["chronograph", "egnyte"],
    requiredPackIds: ["finance-core-pack", "fund-admin-pack"],
  ...
  },
  {
    ...
    description:
      "Review statements and workpapers for evidence support, exceptions, and disclosure risks.",
    icon: "🧾",
    color: "#dc2626",
    category: "finance",
    systemPrompt: `Audit provided statement packages and support files for source support, inconsistencies, missing schedules, disclosure risks, and review exceptions. Produce a findings memo and source-ledger.json. This is audit preparation only, not an audit opinion. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true },
    skills: [
      "finance-source-ledger",
  ...
  },
  {
    ...
    color: "#9333ea",
    category: "finance",
    systemPrompt: `Screen KYC documents using read-only evidence. Extract entity and beneficial ownership facts, apply the configured rules grid, list missing documents and exceptions, and pause for compliance review. Never approve onboarding or represent legal advice. ${FINANCE_GUARDRAIL_PROMPT}`,
    executionMode: "team",
    runtimeDefaults: { autonomousMode: true, allowUserInput: true },
    skills: [
      "finance-source-ledger",
