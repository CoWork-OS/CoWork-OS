...
import { validateDaily, validateEnrollment } from "../../../../services/pulse-worker/src/index";
...
describe("Pulse Worker closed schemas", () => {
  it("accepts valid enrollment and daily payloads", () => {
    expect(
      validateEnrollment({
        schemaVersion: 1,
  ...
  it("rejects content fields and unbounded counts", () => {
    expect(validateDaily({ ...daily, prompt: "must never be accepted" })).toBe(
      "unknown_or_missing_field",
    );
    expect(
      validateDaily({
        ...daily,
        activity: { ...daily.activity, usefulTasks: 100_001 },
      }),
    ).toBe("invalid_usefulTasks");
  });
});
