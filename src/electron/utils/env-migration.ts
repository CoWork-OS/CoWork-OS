...
function shouldWriteValue(existing: unknown, next: string | undefined, mode: EnvSettingsImportMode): boolean {
...
function isProviderConfigured(providerType: LLMProviderType, settings: any): boolean {
  switch (providerType) {
    ...
    case 'openai':
      return !!(normalizeEnvValue(settings?.openai?.apiKey) || normalizeEnvValue(settings?.openai?.accessToken));
    case 'gemini':
    ...
    case 'azure': {
      ...
      const hasDeployment = !!normalizeEnvValue(settings?.azure?.deployment)
    ...
    case 'bedrock':
      return !!(normalizeEnvValue(settings?.bedrock?.accessKeyId) || normalizeEnvValue(settings?.bedrock?.profile));
    case 'ollama':
      return !!(normalizeEnvValue(settings?.ollama?.baseUrl) || normalizeEnvValue(settings?.ollama?.model));
    case 'pi':
      return !!(normalizeEnvValue(settings?.pi?.apiKey) && normalizeEnvValue(settings?.pi?.provider));
    default:
      return false;
  }
}
...
function pickProviderFromSettings(settings: any): LLMProviderType | null {
  if (normalizeEnvValue(settings?.openai?.apiKey) || normalizeEnvValue(settings?.openai?.accessToken)) return 'openai';
  ...
  if (
    ...
    && (normalizeEnvValue(settings?.azure?.deployment) || (Array.isArray(settings?.azure?.deployments) && settings.azure.deployments.length > 0))
  ) return 'azure';
  ...
  if (normalizeEnvValue(settings?.bedrock?.accessKeyId) || normalizeEnvValue(settings?.bedrock?.profile)) return 'bedrock';
  if (normalizeEnvValue(settings?.ollama?.baseUrl) || normalizeEnvValue(settings?.ollama?.model)) return 'ollama';
  if (normalizeEnvValue(settings?.pi?.apiKey) && normalizeEnvValue(settings?.pi?.provider)) return 'pi';
  return null;
}
 ...
 */
export async function importProcessEnvToSettings(options: ImportProcessEnvOptions = {}): Promise<MigrationResult> {
  const migratedKeys: string[] = [];
  ...
  try {
    ...
    const awsRegion = normalizeEnvValue(process.env.AWS_REGION) || normalizeEnvValue(process.env.AWS_DEFAULT_REGION);
    ...
    if (shouldWriteBedrock && (awsAccessKeyId || awsSecretAccessKey || awsSessionToken || awsRegion || awsProfile)) {
      llmSettings.bedrock = {
        ...llmSettings.bedrock,
    ...
    const geminiApiKey = normalizeEnvValue(process.env.GEMINI_API_KEY) || normalizeEnvValue(process.env.GOOGLE_GENERATIVE_AI_API_KEY);
    ...
    const kimiApiKey = normalizeEnvValue(process.env.KIMI_API_KEY) || normalizeEnvValue(process.env.MOONSHOT_API_KEY);
    ...
    if (providerOverride) {
      ...
      const current = (llmSettings.providerType || originalProviderType || 'anthropic') as LLMProviderType;
