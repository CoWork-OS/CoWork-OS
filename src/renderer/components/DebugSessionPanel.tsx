...
export function DebugSessionPanel({ events }: DebugSessionPanelProps) {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      if (!ingest && e.type === "timeline_evidence_attached" && payload?.debugIngestUrl) {
        ...
        if (
          Array.isArray(refs) &&
          refs[0] &&
          typeof (refs[0] as { sourceUrlOrPath?: string }).sourceUrlOrPath === "string"
        ) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: "0.75rem", color: "var(--color-text-muted, #6b7280)", lineHeight: 1.45 }}
      >
        Hypothesize → instrument → reproduce → analyze logs → targeted fix → verify → remove{" "}
        <code style={{ fontSize: "0.7rem" }}>cowork-debug</code> markers. Use structured prompts
        when the agent asks you to reproduce or confirm.
      </div>
      {ingestUrl ? (
