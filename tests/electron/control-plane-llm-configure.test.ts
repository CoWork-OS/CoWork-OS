...
} from "../../src/electron/control-plane/llm-configure";
...
describe("control-plane llm-configure", () => {
  beforeEach(() => {
    ...
    mockFactory.getConfigStatus.mockReturnValue({
      ...
      providers: [{ type: "openai", name: "OpenAI", configured: true }],
    });
  });
  ...
  it("configures openai api key and model", () => {
    ...
    expect(mockFactory.applyModelSelection).toHaveBeenCalledWith(
      expect.any(Object),
      "gpt-4.1-mini",
    );
    expect(mockFactory.saveSettings).toHaveBeenCalledWith(
