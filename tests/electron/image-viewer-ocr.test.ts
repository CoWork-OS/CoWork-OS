...
describe("image-viewer-ocr", () => {
  ...
  it("should decide OCR only for supported raster images", async () => {
    const { shouldRunImageOcr, OCR_SUPPORTED_IMAGE_EXTENSIONS, MAX_IMAGE_OCR_SIZE } =
      await loadModule();
    expect(
      shouldRunImageOcr({ enableImageOcr: true, extension: ".png", fileSizeBytes: 1024 }),
    ).toBe(true);
    expect(
      shouldRunImageOcr({ enableImageOcr: true, extension: ".svg", fileSizeBytes: 1024 }),
    ).toBe(false);
    expect(
      shouldRunImageOcr({
        enableImageOcr: true,
        extension: ".png",
        fileSizeBytes: MAX_IMAGE_OCR_SIZE + 1,
      }),
    ).toBe(false);
    expect(
      shouldRunImageOcr({ enableImageOcr: false, extension: ".png", fileSizeBytes: 1024 }),
    ).toBe(false);
  ...
  it("should execute tesseract once for probe and once for OCR text extraction", async () => {
    ...
    execFileMock.mockImplementation(
      (file: any, args: any, maybeOptions: any, maybeCallback: any) => {
  ...
  it("should return null when tesseract probe fails", async () => {
    ...
    const execFileMock = vi.mocked(childProcess.execFile);
    execFileMock.mockImplementation(
      (_file: any, _args: any, maybeOptions: any, maybeCallback: any) => {
        const callback = typeof maybeOptions === "function" ? maybeOptions : maybeCallback;
