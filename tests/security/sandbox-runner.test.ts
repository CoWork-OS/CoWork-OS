...
describe("SandboxRunner", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should refuse execution when no OS sandbox profile is available", async () => {
      const internals = runner as unknown as {
        generateSandboxProfile: () => string | undefined;
      };
      internals.generateSandboxProfile = () => undefined;

      const result = await runner.execute("echo", ["should-not-run"], {
        cwd: workspace.path,
      });
    ...
    it("should refuse direct execution on platforms without sandbox-exec", async () => {
...
describe("Sandbox Profile Generation", () => {
  it.each(["workspace-write", "read-only"] as const)(
    "uses a private runtime temp directory for the %s named sandbox mode",
    async (accessSandboxMode) => {
      const scopedWorkspace = createMockWorkspace({
        accessProfileId: `named-${accessSandboxMode}`,
        accessSandboxMode,
      });
      const scopedRunner = new SandboxRunner(scopedWorkspace);

      try {
        await scopedRunner.initialize();
        const internals = scopedRunner as unknown as {
          generateSandboxProfile: (allowNetwork?: boolean) => string;
        };
        const profile = internals.generateSandboxProfile(false);
        ...
        scopedRunner.cleanup();
        fs.rmSync(scopedWorkspace.path, { recursive: true, force: true });
      }
    },
  );
  ...
  describe("Network restrictions", () => {
    it("should restrict network when permission is false", async () => {
      ...
      const result = await runner.execute(
        "curl",
        ["--connect-timeout", "1", "http://example.com"],
        {
          timeout: 3000,
        },
      );
